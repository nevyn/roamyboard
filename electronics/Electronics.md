...
Generated files in `KeyModule/fab/` carry a revision in their name (`KeyModule-v5-gerbers.zip`, `KeyModule-v5-bom.csv`); the same revision is in the KiCad title block and on the back silkscreen below the last key. Bump it every time a new set is generated, even for a small change, so an upload can be matched to a file.
